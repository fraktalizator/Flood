...
import com.fraktalizator.flood.componentes.HoverHandlingComponent
...
class EntityInputManager(private val floodScreenViewport: Viewport, private val gameWorld: GameWorld) : InputProcessor {
    ...
    override fun touchUp(screenX: Int, screenY: Int, pointer: Int, button: Int): Boolean {
        val clickedEnt: RenderAbleEntity = getRenderAbleEntityByScreenClick(screenX.toFloat(), screenY.toFloat()) ?: return false;

        if (!clickedEnt.getComponent(RenderComponent::class.java).isVisible) return false

        val touchComp: TouchHandlingComponent = clickedEnt.getComponent(TouchHandlingComponent::class.java) ?: return false
    ...
    private fun getRenderAbleEntityByScreenClick(x:Float, y:Float): RenderAbleEntity? = getRenderAbleEntityByScreenClick(Vector2(x, y))
    private fun getRenderAbleEntityByScreenClick(clickPos:Vector2): RenderAbleEntity?{
        ...
        return gameWorld.entities[clickedTile]
    }
}
